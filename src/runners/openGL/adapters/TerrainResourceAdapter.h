...
public:
  ...
  virtual std::vector<Resource*> doLoad(ResourceLoadRequest &request) const override {
    ...
    while ((token = jsonParser.readToken()) != END_OBJECT && token != FileParser::eof) {
      ...
      if (token == "r") {
        resource->setR(
            (TextureResource*) this->getResourceManager().load(request.relativeUri(jsonParser.readString()), MimeTypes::TEXTURE));
      } else if (token == "g") {
        resource->setG(
            (TextureResource*) this->getResourceManager().load(request.relativeUri(jsonParser.readString()), MimeTypes::TEXTURE));
      } else if (token == "b") {
        ...
        resource->setModel(
            (VertexArrayResource*) this->getResourceManager().load(request.relativeUri(jsonParser.readString()), MimeTypes::VERTEXARRAY));
      } else if (token == "heightmap") {
        resource->setHeightmap(
            (HeightMapResource*) this->getResourceManager().load(request.relativeUri(jsonParser.readString()), MimeTypes::HEIGHTMAP));
