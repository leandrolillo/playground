...
protected:
   ...
   * playback methods
   */
  void playSource(unsigned int sourceId) override
  {
  ...
  void stopSource(unsigned int sourceId) override
  {
  ...
  void pauseSource(unsigned int sourceId) override
  {
   ...
   * listener methods
   */
  bool updateListener(const vector &position, const vector velocity = vector(0, 0, 0), vector to = vector(0, 0, 1),
      vector up = vector(0, 1, 0)) override
      {
