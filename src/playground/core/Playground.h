...
public:
	...
	Chronometer &getStopWatch() {
		if(this->stopWatch == null) {
			throw std::domain_error("StopWatch not registered - you need to include at least one playground runner that provides a stopwatch");
		}
	...
	virtual bool initRunners() {
		...
		for (auto &currentRunner : runners) {
		    try {
                ...
                logger->error("Failed to initialize runner [%d]: [%s]", currentRunner->getId(), exception.what());
                return false;
            } catch(...) {
                logger->error("Failed to after initialize runner [%d]", currentRunner->getId());
                return false;
